...
public class ProjectionTests {
    ...
    /*
    * 프로젝션
    * select절에 조회할 대상을 지정하는것을 프로젝션이라한다
    * (select{프로젝션대상} from...)
    *
    * 1. 엔티티 프로젝션
    * - 원하는 객체를 바로 조회할수있다.
    ...
    *
    * 2. 임베디드타입 프로젝션
    * - 임베디드타입은 영속성 컨텍스트에서 관리되지 않는다
    * - 엔티티랑 거의 비슷하게 사용된다(조회의 시작점이 될수없다)->from에 사용불가
    *
    * 3. 스칼라타입 프로젝션
    * - 숫자,문자,날짜같은 기본 데이터 타입
    * - 스칼라타입은 영속성 컨텍스트에서 관리되지 않는다.
    *
    * 4. new명령어를 활용한 프로젝션
    * - 다양한 종류의 단순 값들을 DTO로 조회하는 방식(new 패키지명,dto명)
    * - > new를 사용해서 만들기때문에 기본생성자가 필요하다.
    * - new명령어를 사용한 클래스는 엔티티가 아니므로 영속성컨텍스트에서 관리되지 않음
    *
    * */
    ...
    @Test
    @DisplayName("TypeQuery를 이용한 스칼라타입 프로젝션 테스트")
    void test4(){
        //given
        //when
    ...
    @Test
    @DisplayName("new명령어를 활용한 프로젝션 테스트")
    void test6(){
